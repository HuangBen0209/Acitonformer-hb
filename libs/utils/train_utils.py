...
def valid_one_epoch(val_loader, model, curr_epoch, ext_score_file=None, evaluator=None, output_file=None, tb_writer=None, print_freq=20):
    ...
    results['t-start'] = torch.cat(results['t-start']).numpy().tolist()
    results['t-end'] = torch.cat(results['t-end']).numpy().tolist()
    results['label'] = torch.cat(results['label']).numpy().tolist()
    results['score'] = torch.cat(results['score']).numpy().tolist()
